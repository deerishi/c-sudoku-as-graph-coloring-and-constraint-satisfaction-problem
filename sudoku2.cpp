...
void search(unordered_map<int,unordered_set<int> >grid,int assignSquare,unordered_set<int> remaining)
{
    ...
    for(it=grid[assignSquare].begin();it!=grid[assignSquare].end() and solved==false;it++)// since the grid contains all the possible numbers that can fit into the square
    {
...
int main()
{
    ...
    string name="problems/"+to_string(nm)+"/"+to_string(co)+".sd";
    int c;
    char name2[50];
    grid.clear();
